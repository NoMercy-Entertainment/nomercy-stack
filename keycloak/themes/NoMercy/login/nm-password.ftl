...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('password'); section>
    <#if section = "header">
        <span class="nm-head">
            ...
            <#if maskedEmail??>
                <span class="nm-head__sub">${msg("nmPasswordSub")} <strong>${kcSanitize(maskedEmail)?no_esc}</strong></span>
            </#if>
        </span>
    <#elseif section = "form">
        <form id="kc-form-login" class="nm-form" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
            ...
            <#if allowCode!false>
            <p class="nm-alt"><button class="nm-linkbtn" name="useCode" type="submit">${msg("nmUseCode")}</button></p>
            </#if>
        </form>
    </#if>
